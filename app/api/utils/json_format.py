...
def json_response_for_report(prepared_data: list[Driver]) -> Response:
...
def json_response_for_drivers(prepared_data: list[Driver]) -> Response:
...
def json_response_for_driver(driver: Driver) -> Response:
    data = {driver.name: _prepare_json_driver(driver)}
